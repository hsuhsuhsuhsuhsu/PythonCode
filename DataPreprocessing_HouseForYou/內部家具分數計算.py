...
Total_equip = []
n=2943
with open ("已刪圖的內部 -內部分數用.csv","r") as f:
    for line in f :  
        row = []     
        temp = 0
...
for HouseNumber in house[1:]:     
    ...
    print(HouseNumber[1:])
    for equip in HouseNumber[1:]:#原本是  HouseNumber[0:]但是第0個是編號 不是家具
        temp.append(equip)
...
for HouseNumber in house[1:]:
    ...
    #tfidfPerHouse.append(temp)
    #print("TFPDF加總="+str(tempp))  
    #tfpdfPerHouse.append(tempp)
    #cp.append(temp+tempp)

#with open("aaaaaaa.csv","w",encoding="big5",newline="") as fout:
    #csvWriter = csv.writer(fout) 
   # for row1,row2 in zip(tfidfPerHouse,tfpdfPerHouse) :
        #csvWriter.writerow([row1,row2])
        
with open("house.csv","w",encoding="big5",newline="") as fout:
    csvWriter = csv.writer(fout) 
    for row1,row2 in zip(TFIDF,TFPDF) :
        csvWriter.writerow([row1,row2])        
