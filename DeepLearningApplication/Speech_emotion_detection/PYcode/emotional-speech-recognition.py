...
#兩個csv是作者run過一遍已經抓過特徵的data frame 直接讀就不用去抓特徵了
DATA_FRAMES = True
fem_path = r'C:\Users\hsu\Desktop\FINAL_Speech_emotion_detection\MFCCs_for_Speech_Emotion_Recognition\Female_features.csv'
mal_path = r'C:\Users\hsu\Desktop\FINAL_Speech_emotion_detection\MFCCs_for_Speech_Emotion_Recognition\Male_features.csv'
#%%
#Emotions Speech datasets
#Content Data set contains files from RAVDESS speechs, CREMA-D, SAVEE, TESS.
os.getcwd()
os.chdir(r"C:\Users\hsu\Desktop\FINAL_Speech_emotion_detection")
...
#path = path = 'ravdess-emotional-speech-audio/audio_speech_actors_01-24/Actor_01/03-01-05-01-01-01-01.wav'
...
from tensorflow.keras.utils import to_categorical #delete np_utils,
...
# print(model_build_summary(x_train.shape[1], x_train, x_test, y_test))
...
print((end - start),"sec")#6783.62969620002 sec
...
for file in dir:
    ...
    WAV_test.append([emotion, path]) 
  
WAV_test_df = pd.DataFrame(WAV_test)
WAV_test_df.columns = ['labels', 'path']
print('WAV_test datasets')
WAV_test_df.head()

WAV_test_X, WAV_test_Y = [], []
for path, emotion in zip(WAV_test_df.path, WAV_test_df.labels):
    features = get_features(path)
    #adding augmentation, get_features return a multi dimensional array (for each augmentation), so we have to use a loop to fill the df
    for elem in features: 
        WAV_test_X.append(elem)        
        WAV_test_Y.append(emotion)

Wav_X = WAV_test_X #size = 3745 * 58
Wav_Y = WAV_test_Y #size = univariate 3745

Wav_X = scaler.transform(Wav_X)
Wav_X = np.expand_dims(Wav_X, axis=2)

#Wav_y = size(3745, 7)
#716 only for encoder
# encoder = OneHotEncoder(categories = ["anger", "calm", "disgust", "fear", "happy", "sad", "neutral", "surprise"])
# encoder.categories_
Wav_Y = encoder.fit_transform(np.array(Wav_Y).reshape(-1,1)).toarray()

pred_test = total_model.predict(Wav_X) #predict using total_model -> Wav_X

#####################start adj

myWav_X = WAV_test_X * 1 #size = 3745 * 58
myWav_Y = WAV_test_Y * 1 #size = univariate 3745

adj = myWav_X[1]#random
adj[1] = adj[1]/2 #random

myWav_X.append(adj)
myWav_Y.append("surprise")

myWav_X = scaler.transform(myWav_X)
myWav_X = np.expand_dims(myWav_X, axis=2)

#Wav_y = size(3745, 7)
#716 only for encoder
#myEncoder= OneHotEncoder(categories = ["anger", "calm", "disgust", "fear", "happy", "sad", "neutral", "surprise"])
myEncoder= OneHotEncoder()
temp3 = np.array(myWav_Y).reshape(-1, 1)
#myWav_Y = myEncoder.fit_transform(temp3).toarray()
#myEncoder.fit(np.array(myWav_Y).reshape(-1, 1))
myWav_Y = myEncoder.fit_transform(temp3).toarray()
# myEncoder._check_X(myWav_Y)[2]
# myEncoder.categories_


mypred_test = total_model.predict(myWav_X) #predict using total_model -> Wav_X





#生氣 冷靜 噁心 害怕 快樂 中性 難過 驚喜
#myWeight = [0.1, 0, 0.1, 0.45, 0.25, 0, 0.1]
# myWeight = [1/7, 1/7, 1/7, 1/7, 1/7, 1/7, 1/7]
# temp = []
# for i in range(len(pred_test[:, 7])):
#     temp.append(np.array(pred_test[i, 7]) * np.array(myWeight))
    
# temp = np.array(temp)
# temp2 = pred_test[:, 0:7] + temp
...
#     M = np.where(pred_test[i,:]==np.max(pred_test[i,:]))
...
# outcome = np.hstack((out,y_test_))  
    




#A = encoder.fit_transform(np.array(pred_test).reshape(-1,1)).toarray()

#y_pred = encoder.inverse_transform(pred_test)
#y_pred = encoder.inverse_transform(temp2)
y_pred = myEncoder.inverse_transform(mypred_test)
#y_test_ = encoder.inverse_transform(Wav_Y)
y_test_ = myEncoder.inverse_transform(myWav_Y)


sum((y_pred==y_test_)*1)


#%%
#TEST DATA
encoder = OneHotEncoder()
RAV_TEST ="C:/Users/hsu/Desktop/FINAL_Speech_emotion_detection/Test/"
dir = os.listdir(RAV_TEST)
test=[]         
files = os.listdir(RAV_TEST)
for file in files: 
    part = file.split('.')[0]
    part = part.split("-")              
    ...
    path = (RAV_TEST + '/' + file)
    #emotion = 'female_'+emotion
    test.append([emotion, path]) 
         
test_df = pd.DataFrame(test)
test_df.columns = ['labels', 'path']
print('TEST datasets')
test_df.head()


test_X, test_Y = [], []
for path, emotion in zip(test_df.path, test_df.labels):
    features = get_features(path)
    #adding augmentation, get_features return a multi dimensional array (for each augmentation), so we have to use a loop to fill the df
    for elem in features: 
        test_X.append(elem)        
...
#pred_test = total_model.predict(test_X)
mpred_test = male_model.predict(test_X)

test_Y = encoder.fit_transform(np.array(test_Y).reshape(-1,1)).toarray()
y_pred = encoder.inverse_transform(mpred_test)
y_test_ = encoder.inverse_transform(test_Y)
...
print("Test")
df.head(20)
#sns.countplot(test_df.labels, order = order)
